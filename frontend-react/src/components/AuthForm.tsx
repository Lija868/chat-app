...
export default function AuthForm({ onLoggedIn }: { onLoggedIn: (token: string) => void }){
  ...
  async function login(){
    ...
    const r = await fetch(API+'/token', { method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: params.toString() })
    const j = await r.json(); if(j.access_token){ onLoggedIn(j.access_token) } else { alert('Login failed') }
  }
  return (
